...
echo =======================================================
echo.
ssh root@72.62.91.101 "cd gestionale && pm2 stop gestionale && (mv prisma/gestionale.db prisma/gestionale.db.bak || true) && git fetch --all && git reset --hard origin/main && (mv prisma/gestionale.db.bak prisma/gestionale.db || true) && chmod -R 777 prisma && npm install && npx prisma db push --accept-data-loss && npx prisma generate && rm -rf .next && npm run build && pm2 restart gestionale"
